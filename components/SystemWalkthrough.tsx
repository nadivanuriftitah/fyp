...
const StepButton: React.FC<{ step: WalkthroughStep, onClick: () => void, isActive: boolean }> = ({ step, onClick, isActive }) => (
    <div className="flex flex-col items-center flex-shrink-0 w-24">
        <button
            onClick={onClick}
            className={`w-16 h-16 rounded-full flex items-center justify-center border-2 transition-all duration-300 btn-shimmer ${isActive ? 'bg-[#1EE3CF] border-[#1EE3CF] shadow-lg shadow-[#1EE3CF]/30' : 'bg-[#1B263B] border-[#415A77] hover:border-[#1EE3CF]'}`}
        >
